<div class="order-3 lg:order-3 rounded-[1.8rem] bg-gradient-to-b from-[#151c2b] to-[#0f1724] border border-white/10 p-5">
    <div class="h-28 rounded-3xl border border-white/10 mb-5 flex items-center justify-end gap-5 px-6 bg-cover bg-center"
        style="
                            @if ($match->team_b_background) background-image: linear-gradient(rgba(0,0,0,.25), rgba(0,0,0,.25)), url('{{ '/storage/' . $match->team_b_background }}');
                            @else
                                background-color: {{ $match->team_b_color ?? '#dc2626' }}; @endif
                        ">
        ...
        <div class="w-16 h-16 rounded-full bg-white/20 flex items-center justify-center overflow-hidden shrink-0">
            ...
            @else
                <svg viewBox="0 0 64 64" class="w-12 h-12 drop-shadow">
                    <path fill="{{ $match->team_b_jersey_color ?? '#dc2626' }}"
    ...
    <div class="text-center my-8">
        <div id="team-b-score" class="text-8xl md:text-9xl font-black leading-none">
            {{ $match->team_b_score }}
        </div>
    </div>
